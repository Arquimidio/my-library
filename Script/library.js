...
const pagination = (function(){
	...
	/*function createPaginationArray(arr){
		...
		let cut = arr.length % len
      	let chunked = arr.slice(0, arr.length - (cut)).reduce((acc, elt, i) => {
        if(((i + 1) % len === 0)) acc.push(arr.slice(i + 1 - len, i + 1))
        return acc
      }, [])
      return cut? [...chunked, arr.slice(arr.length - cut, arr.length)]:chunked
	}*/
